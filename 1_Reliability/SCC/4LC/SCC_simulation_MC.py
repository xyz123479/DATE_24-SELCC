...
def main() :
    ...
    # This is SCC
    print("-----------------------------------------------------------")
    print(" SCC with 64-bit data, 7-bit parity")
    print("-----------------------------------------------------------")
    ...
    for Nrun in range (MC_SIM_CNT) :
        ...
        corrected_codeword, detectable, correctable = correction_SCC(codeword, syndrome, H_matrix, Sum_matrix)
    ...
    for Nrun in range (MC_SIM_CNT) :
        error_pos1 = random.randrange(codeword_len - 1)  # exclude error on 71th bit
    ...
    for Nrun in range (MC_SIM_CNT) :
        ...
        error_cell_idx1 = error_pos1 // 2
        while (True) :
            error_pos2 = random.randrange(codeword_len - 1)  # exclude error on 71th bit
            error_cell_idx2 = error_pos2 // 2
            if error_cell_idx1 != error_cell_idx2 :
                break
        while (True) :
